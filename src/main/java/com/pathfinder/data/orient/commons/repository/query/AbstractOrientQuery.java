...
public abstract class AbstractOrientQuery implements RepositoryQuery {
    ...
    protected OrientQueryExecution getExecution() {
        ...
        if (method.isCollectionQuery()) {
            return new OrientQueryExecution.CollectionExecution(operations, parameters);
        } else if (isCountQuery()) {
            return new OrientQueryExecution.CountExecution(operations, parameters);
        } else if (method.isPageQuery()) {
            return new OrientQueryExecution.PagedExecution(operations, parameters);
        } else if (method.isQueryForEntity()) {
            return new OrientQueryExecution.SingleEntityExecution(operations, parameters);
        } else if (isDeleteQuery()) {
            return new OrientQueryExecution.DeleteExecution(operations, parameters);
        }
